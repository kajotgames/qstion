...
class QsStringifier(QS):
    ...
    def __init__(
        ...
        if array_format not in LIST_FORMAT_OPTIONS:
            raise ValueError(f"array_format must be one of {LIST_FORMAT_OPTIONS}")
    ...
    def stringify(self, data: dict, filter: list = None) -> list[tuple]:
        ...
        _q_args = {}
        for item in data:
            node = QsNode.load(item, data[item], filter=filter)
            if node is not None:
                self._root_node[node.key] = node
        for item in self._root_node:
            for arg in self._get_arg(item):
                if arg is None:
                    continue
                prepared_arg = self._transform_arg(arg)
                if prepared_arg[0] in _q_args:
                    _q_args[prepared_arg[0]] = (
                        [_q_args[prepared_arg[0]], prepared_arg[1]]
                        if isinstance(_q_args[prepared_arg[0]], str)
                        else [*_q_args[prepared_arg[0]], prepared_arg[1]]
                    )
                else:
                    _q_args[prepared_arg[0]] = prepared_arg[1]
        res = []
...
def stringify(
    ...
    Returns:
        str: query string
    """
    qs = QsStringifier(allow_dots=allow_dots, array_format=array_format)
